---
title: "사일런트 힐: 타운폴 — 안개 낀 섬으로 돌아온 남자"
description: "스크린 번 인터랙티브가 개발하고 안나푸르나 인터랙티브와 코나미 디지털 엔터테인먼트가 배급하는 서바이벌 호러 신작이에요. 주인공 사이먼 오델이 세인트 아멜리아 섬으로 돌아와 안개에 삼켜진 거리와 해안선을 탐험하며 조각난 단서를 따라갑니다. 2026년 플레이스테이션 5와 윈도우로 출시될 예정이에요."
pubDate: 2026-09-05
category: "New"
hook: "돌아오라는 부탁, 안개뿐인 섬"
tags: ["사일런트힐", "서바이벌호러", "심리공포", "PS5", "PC신작", "2026신작"]
enName: "SILENT HILL: Townfall"
thumbnail: "/games/2026-09-05-silent-hill-townfall-thumb.jpg"
customThumb: true
imageCredit:
  text: "© 각 게임사·기관 (공식 이미지)"
  url: "https://store.steampowered.com/app/1636440/"
officialLinks:
  - label: "Steam 상점 페이지"
    url: "https://store.steampowered.com/app/1636440/"
host: "rosie"
views: 0
faq:
  - q: "어떤 장르의 게임인가요?"
    a: "서바이벌 호러예요. 사일런트 힐 시리즈의 계보를 잇는 작품이라고 보시면 됩니다."
  - q: "어느 플랫폼으로 나오나요?"
    a: "플레이스테이션 5와 윈도우로 출시될 예정이에요."
  - q: "언제 출시되나요?"
    a: "2026년 출시 예정이에요. 구체적인 일정은 공식 발표를 확인해 주시는 게 정확해요."
  - q: "누가 만들고 누가 배급하나요?"
    a: "개발은 스크린 번 인터랙티브가 맡고, 배급은 안나푸르나 인터랙티브와 코나미 디지털 엔터테인먼트가 함께 해요."
  - q: "주인공과 배경은 어떻게 되나요?"
    a: "주인공은 사이먼 오델이고, 배경은 안개에 삼켜진 세인트 아멜리아 섬이에요. 조각난 단서를 따라가며 섬과 예전 주민들에 얽힌 이야기를 풀어가게 됩니다."
coupang:
  - title: "게이밍 헤드셋"
    desc: "발소리와 환경음이 중요한 호러 게임은 헤드셋 하나로 체감이 달라져요."
    url: "https://www.coupang.com/np/search?q=%EA%B2%8C%EC%9D%B4%EB%B0%8D%20%ED%97%A4%EB%93%9C%EC%85%8B&src=1139000&spec=10799999&addtag=200&ctag=%EA%B2%8C%EC%9D%B4%EB%B0%8D%20%ED%97%A4%EB%93%9C%EC%85%8B&lptag=AF7838146&pageType=SEARCH&pageValue=%EA%B2%8C%EC%9D%B4%EB%B0%8D%20%ED%97%A4%EB%93%9C%EC%85%8B"
  - title: "PS5 듀얼센스 무선 컨트롤러"
    desc: "진동과 트리거 감각까지 살리려면 여분 컨트롤러가 든든해요."
    url: "https://www.coupang.com/np/search?q=PS5%20%EB%93%80%EC%96%BC%EC%84%BC%EC%8A%A4%20%EC%BB%A8%ED%8A%B8%EB%A1%A4%EB%9F%AC&src=1139000&spec=10799999&addtag=200&ctag=PS5%20%EB%93%80%EC%96%BC%EC%84%BC%EC%8A%A4%20%EC%BB%A8%ED%8A%B8%EB%A1%A4%EB%9F%AC&lptag=AF7838146&pageType=SEARCH&pageValue=PS5%20%EB%93%80%EC%96%BC%EC%84%BC%EC%8A%A4%20%EC%BB%A8%ED%8A%B8%EB%A1%A4%EB%9F%AC"
scenes:
  - text: "안녕하세요, 여러분의 게임 안내자 **로지**예요! 오늘은 안개 속으로 한 발 들어가 볼까요? **사일런트 힐: 타운폴** 이야기입니다."
    image: "/games/scenes/2026-09-05-silent-hill-townfall-1.jpg"
    emotion: "happy"
    chapter: "오프닝"
  - text: "이 게임은 **스크린 번 인터랙티브**가 개발하고, **안나푸르나 인터랙티브**와 **코나미 디지털 엔터테인먼트**가 배급하는 **서바이벌 호러** 신작이에요."
    image: "/games/scenes/2026-09-05-silent-hill-townfall-2.jpg"
    emotion: "idle"
    chapter: "만든 사람들"
  - text: "주인공은 **사이먼 오델**. **함께 문제를 바로잡아 달라**는 부탁을 받고 **세인트 아멜리아 섬**으로 돌아옵니다. 부탁 하나로 시작되는 이야기라니, 벌써 조마조마하죠?"
    image: "/games/scenes/2026-09-05-silent-hill-townfall-3.jpg"
    emotion: "surprised"
    chapter: "돌아온 남자"
  - text: "플레이어는 **안개에 삼켜진 거리와 해안선**을 탐험하게 돼요. 갈 수 있는 곳이 넓다기보다, **보이지 않는 곳이 넓은** 쪽에 가깝겠네요."
    image: "/games/scenes/2026-09-05-silent-hill-townfall-4.jpg"
    emotion: "think"
    chapter: "안개의 섬"
  - text: "단서는 통째로 주어지지 않아요. **조각난 단서**를 하나씩 따라가면서, **나와 마을과 예전 주민들을 얽어맨 불안**의 정체에 다가가는 구조예요."
    image: "/games/scenes/2026-09-05-silent-hill-townfall-5.jpg"
    emotion: "think"
    chapter: "조각난 단서"
  - text: "플랫폼은 **플레이스테이션 5**와 **윈도우**, 출시는 **2026년**이에요. 시리즈를 오래 기다린 분이라면 달력에 표시해 두셔도 좋겠어요."
    image: "/games/scenes/2026-09-05-silent-hill-townfall-6.jpg"
    emotion: "happy"
    chapter: "출시 정보"
  - text: "안개 너머가 궁금해지는 작품이었어요. 로지였습니다."
    image: "/games/2026-09-05-silent-hill-townfall-thumb.jpg"
    emotion: "happy"
    chapter: "엔딩"
---

스크린 번 인터랙티브가 개발하고 안나푸르나 인터랙티브와 코나미 디지털 엔터테인먼트가 배급하는 서바이벌 호러 신작이에요. 주인공 사이먼 오델이 세인트 아멜리아 섬으로 돌아와 안개에 삼켜진 거리와 해안선을 탐험하며 조각난 단서를 따라갑니다. 2026년 플레이스테이션 5와 윈도우로 출시될 예정이에요.

*(이미지는 각 게임사·기관의 공식 스크린샷·홍보 이미지만 사용했습니다.)*
