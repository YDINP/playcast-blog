---
title: "드디어 롤 클래식 7월 29일 출시! 다운로드 없이 '시즌3' 그 시절 협곡으로"
description: "리그 오브 레전드 클래식이 7월 29일(한국 7월 30일) 열려요. 2013 시즌3 감성의 옛 소환사의 협곡, 구 룬·특성, 챔피언 60종에 클래식 패스까지. 별도 다운로드 없이 기존 클라이언트에서 즐겨요."
pubDate: 2026-07-16
updatedDate: 2026-07-22
category: "New"
tags: ["롤클래식", "리그오브레전드", "롤", "LoL", "시즌3", "라이엇"]
thumbnail: "/games/lol-thumb.jpg"
customThumb: true
imageCredit:
  text: "© Riot Games (League of Legends Classic 공식)"
  url: "https://www.leagueoflegends.com/en-us/classic/"
host: "rosie"
views: 0
faq:
  - q: "롤 클래식 출시일은 언제인가요?"
    a: "2026년 7월 29일, 패치 26.15로 열려요. 한국은 지역 서버 점검 일정상 7월 30일 새벽에 적용돼요."
  - q: "롤 클래식 오픈 시간은 몇 시인가요?"
    a: "북미 기준 7월 29일 오전 8시(PDT)에 시작해요. 한국 시각으로는 7월 30일 자정 무렵이며, 지역별 점검 일정에 따라 다소 차이가 있을 수 있어요."
  - q: "롤 클래식은 별도 다운로드가 필요한가요?"
    a: "아니요. 별도 다운로드나 구독 없이 지금 쓰는 리그 오브 레전드 클라이언트에 그대로 포함돼요."
  - q: "챔피언은 몇 종인가요?"
    a: "리메이크 이전 스킬의 챔피언 60종(원년 40명 + 이후 순차 해금)으로 시작해요."
  - q: "클래식 패스가 있나요?"
    a: "네. IP(영향력 포인트) 부활, 1~30레벨 육성, 별도 상점과 함께 클래식 패스가 제공돼요."
  - q: "기존 롤과 무엇이 다른가요?"
    a: "2013 시즌3 감성이에요. 옛 소환사의 협곡, 구 룬·특성 시스템, 그 시절 아이템과 리메이크 이전 챔피언 스킬을 되살렸어요."
coupang:
  - title: "게이밍 마우스"
    desc: "정확한 조작엔 게이밍 마우스"
    url: "https://www.coupang.com/np/search?q=%EA%B2%8C%EC%9D%B4%EB%B0%8D%20%EB%A7%88%EC%9A%B0%EC%8A%A4&src=1139000&spec=10799999&addtag=200&ctag=%EA%B2%8C%EC%9D%B4%EB%B0%8D%20%EB%A7%88%EC%9A%B0%EC%8A%A4&lptag=AF7838146&pageType=SEARCH&pageValue=%EA%B2%8C%EC%9D%B4%EB%B0%8D%20%EB%A7%88%EC%9A%B0%EC%8A%A4"
  - title: "게이밍 기계식 키보드"
    desc: "빠른 입력·손맛"
    url: "https://www.coupang.com/np/search?q=%EA%B2%8C%EC%9D%B4%EB%B0%8D%20%EA%B8%B0%EA%B3%84%EC%8B%9D%20%ED%82%A4%EB%B3%B4%EB%93%9C&src=1139000&spec=10799999&addtag=200&ctag=%EA%B2%8C%EC%9D%B4%EB%B0%8D%20%EA%B8%B0%EA%B3%84%EC%8B%9D%20%ED%82%A4%EB%B3%B4%EB%93%9C&lptag=AF7838146&pageType=SEARCH&pageValue=%EA%B2%8C%EC%9D%B4%EB%B0%8D%20%EA%B8%B0%EA%B3%84%EC%8B%9D%20%ED%82%A4%EB%B3%B4%EB%93%9C"
scenes:
  - text: "롤 유저 주목! '**리그 오브 레전드 클래식**'이 드디어 열려요. **2013년 시즌3**, 그 시절 감성 그대로예요."
    image: "/games/scenes/lol-classic.png"
    emotion: "surprised"
    chapter: "오프닝"
  - text: "출시는 **7월 29일**, 한국은 7월 30일이에요. 26.15 패치로 열리고, **별도 다운로드 없이** 지금 쓰는 클라이언트에서 바로 즐겨요."
    image: "/games/scenes/lol-launch.jpg"
    emotion: "happy"
    chapter: "① 언제·어떻게"
  - text: "무대는 옛 **소환사의 협곡**이에요. 구 룬·특성 시스템, 그 시절 아이템, 리메이크 전 챔프 스킬까지 되살렸어요."
    image: "/games/scenes/lol-classic-runes.png"
    emotion: "idle"
    chapter: "② 시즌3 복각"
  - text: "정확히 한 패치를 재현한 건 아니래요. 2013 시즌3를 뼈대로 초창기 '**명장면**'들을 모은 컬렉션에 가까워요."
    image: "/games/scenes/lol-hits.jpg"
    emotion: "think"
    chapter: "③ 그리팅스 히츠"
  - text: "챔피언은 **60종**으로 시작해요. 원년 40명에 더해 시간이 지나며 더 풀리는 방식이고요."
    image: "/games/scenes/lol-champs.jpg"
    emotion: "happy"
    chapter: "④ 챔피언 60"
  - text: "**영향력 포인트(IP)**가 돌아오고, 1~30 레벨과 별도 상점, **클래식 패스**도 있어요. 느리고 전략적인 그때 그 템포로요."
    image: "/games/scenes/lol-system.jpg"
    emotion: "idle"
    chapter: "⑤ 그때 그 시스템"
  - text: "추억 소환할 준비 됐나요? **별도 구독도 없어요**. 열리면 다시 정리해서 물어올게요!"
    image: "/games/lol-thumb.jpg"
    emotion: "happy"
    chapter: "마무리"
    holdMs: 2000
---

리그 오브 레전드 클래식이 **7월 29일(한국 7월 30일)**, 패치 26.15로 열려요. **2013 시즌3**를 뼈대로 옛 **소환사의 협곡**, 구 **룬·특성 시스템**, 그 시절 아이템과 리메이크 이전 챔피언 스킬을 되살린 모드예요. 한 패치를 그대로 복원하기보다 초창기 '명장면'을 모은 컬렉션에 가깝고, **챔피언 60종**(원년 40명 + 이후 추가 해금)으로 시작해요. **영향력 포인트(IP)** 부활, **1~30 레벨** 육성, 별도 상점, **클래식 패스**까지 그 시절 시스템을 함께 담았어요. 별도 다운로드나 구독 없이 **기존 클라이언트**에 포함돼요. (공식 발표 기준으로 정리했어요.)
